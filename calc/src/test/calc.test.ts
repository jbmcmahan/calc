...
describe('calc', () => {
  describe('Multi-Gen', () => {
    ...
    inGens(8, 8, ({gen, calculate, Pokemon, Move}) => {
        ...
        for (const move of [Move('Seismic Toss'), Move('Night Shade')]) {
          ...
          expect(result.desc()).toBe(
            gen < 3
              ...
              : `Lvl 50 Mew ${move.name} vs. 31 HP Vulpix: 50-50 (23 - 23%) -- guaranteed 5HKO`
          );
        }
      });
    });
    ...
    inGens(8, 8, ({gen, calculate, Pokemon, Move, Field}) => {
    ...
    inGens(8, 8, ({gen, calculate, Pokemon, Move, Field}) => {
      test(`Critical hits ignore attack decreases (gen ${gen})`, () => {
        ...
        if (gen < 2) {
          ...
          expect(result.desc()).toBe(
            '31 Atk burned Mew Explosion vs. 31 HP / 31 Def Vulpix on a critical hit: 545-642 (251.1 - 295.8%) -- guaranteed OHKO'
          );
        }
    ...
    inGens(8, 8, ({gen, calculate, Pokemon, Move, Field}) => {
      test(`Weather Ball should change type depending on the weather (gen ${gen})`, () => {
        ...
        for (const {weather, type, damage} of weathers) {
          ...
          expect(result.desc()).toBe(
            `31 ${atk} Castform Weather Ball (100 BP ${type}) vs. 31 HP / 31 ${def} Bulbasaur in ${weather}: ${dmg.range[0]}-${dmg.range[1]} ${dmg.desc}`
          );
        }
      });
    });
    ...
    inGens(8, 8, ({gen, calculate, Pokemon, Move}) => {
      test(`Thousand Arrows and Ring Target Should negate damage nullfiers (gen ${gen})`, () => {
        ...
        expect(result.desc()).toBe(
          '31 Atk Zygarde Thousand Arrows vs. 31 HP / 31 Def Swellow: 147-174 (56.3 - 66.6%) -- guaranteed 2HKO'
        );
      });
    });
    ...
    inGens(8, 8, ({gen, calculate, Pokemon, Move}) => {
      ...
      if (gen === 4) {
        test(`Iron Ball negates ground immunities (gen ${gen})`, () => {
          ...
          expect(result.desc()).toBe(
            '31 Atk Vibrava Earthquake vs. 31 HP / 31 Def Zapdos: 186-218 (57.9 - 67.9%) -- guaranteed 2HKO'
          );
        });
      } else {
        test(`Iron Ball Should negate damage nullifiers (gen ${gen})`, () => {
          ...
          expect(result.desc()).toBe(
            '31 Atk Vibrava Earthquake vs. 31 HP / 31 Def Zapdos: 93-109 (28.9 - 33.9%) -- 1.2% chance to 3HKO'
          );
        });
      }
      test(`Iron Ball negates levitate (gen ${gen})`, () => {
        ...
        expect(result.desc()).toBe(
          '31 SpA Poliwrath Mud Shot vs. 31 HP / 31 SpD Mismagius: 29-35 (11.1 - 13.4%) -- possible 8HKO'
        );
      });
    });
    ...
    inGens(8, 8, ({gen, calculate, Pokemon, Move, Field}) => {
      test(`Psychic Terrain (gen ${gen})`, () => {
        ...
        if (gen < 8) {
          ...
          expect(result.desc()).toBe(
            '+2 31 SpA Mewtwo Psystrike vs. 31 HP / 31+ Def Marvel Scale Milotic in Psychic Terrain: 331-391 (84.2 - 99.4%) -- guaranteed 2HKO after burn damage'
          );
        } else {
          expect(result.range()).toEqual([331, 391]);
          expect(result.desc()).toBe(
    ...
    inGens(8, 8, ({gen, calculate, Pokemon, Move}) => {
      test(`Parental Bond (gen ${gen})`, () => {
        ...
        if (gen === 6) {
          ...
          expect(result.desc()).toBe(
            '31 Atk Parental Bond Kangaskhan-Mega Frustration vs. 31 HP / 31+ Def Amoonguss: 190-225 (43.9 - 52%) -- approx. 6.6% chance to 2HKO'
          );
        }
        ...
        expect(result.desc()).toBe(
          'Lvl 88 Parental Bond Kangaskhan-Mega Seismic Toss vs. 31 HP Amoonguss: 176-176 (47.6 - 47.6%) -- guaranteed 3HKO'
        );
        ...
        if (gen === 6) {
          ...
          expect(result.desc()).toBe(
            '31 Atk Parental Bond Kangaskhan-Mega Power-Up Punch vs. Lvl 72 31 HP / 31 Def Aggron: 196-236 (95.6 - 115.1%) -- 78.9% chance to OHKO'
          );
        }
        ...
        expect(result.desc()).toBe(
          '31 Atk Parental Bond Kangaskhan-Mega Crunch vs. 31 HP / 31 Def Shadow Shield Lunala: 280-334 (67.4 - 80.4%) -- approx. 2HKO'
        );
      });
    });
  });
  ...
  describe('Gen 8', () => {
    inGen(8, ({calculate, Pokemon, Move, Field}) => {
      test('Basic: Gengar vs. Chansey', () => {
        ...
        expect(result.range()).toEqual([204, 242]);
        expect(result.desc()).toBe(
          '+3 31+ SpA Life Orb Gengar Sludge Bomb vs. +1 31 HP / 31 SpD Eviolite Chansey: 204-242 (30.6 - 36.3%) -- 52.9% chance to 3HKO'
        );
      });
      ...
      test('Knock Off vs. Silvally', () => {
        ...
        expect(result.desc()).toBe(
          '31 Atk Mold Breaker Sawk Knock Off vs. 31 HP / 31 Def Silvally-Dark: 36-43 (10.8 - 12.9%) -- possible 8HKO'
        );
      });
      ...
      test('-ate Abilities', () => {
        ...
        expect(result.desc()).toBe(
          '31 SpA Pixilate Sylveon Hyper Voice vs. 31 HP / 31 SpD Silvally: 165-195 (49.8 - 58.9%) -- 99.6% chance to 2HKO'
        );
      });
      ...
      test('% chance to OHKO', () => {
        ...
        expect(result.desc()).toBe(
          'Lvl 55 31 SpA Choice Specs Abomasnow Blizzard vs. 31 HP / 31 SpD Deerling: 236-278 (87.4 - 102.9%) -- 25% chance to OHKO'
        );
      });
      ...
      test('% chance to OHKO with Leftovers', () => {
        ...
        expect(result.desc()).toBe(
          '31 SpA Choice Specs Kyurem Earth Power vs. 31 HP / 31 SpD Jirachi: 294-348 (86.2 - 102%) -- 12.5% chance to OHKO'
        );
      });
      ...
      test('Technician with Low Kick', () => {
        ...
        expect(result.range()).toEqual([272, 320]);
        expect(result.desc()).toBe(
          '31 Atk Technician Ambipom Low Kick (60 BP) vs. 31 HP / 31 Def Blissey: 272-320 (75.1 - 88.3%) -- guaranteed 2HKO'
        );
        ...
        expect(result.range()).toEqual([112, 132]);
        expect(result.desc()).toBe(
          '31 Atk Ambipom Low Kick (120 BP) vs. 31 HP / 31 Def Aggron: 112-132 (63.2 - 74.5%) -- guaranteed 2HKO'
        );
      });
    });
    ...
    describe('Gen 9', () => {
      inGen(9, ({calculate, Pokemon, Move}) => {
        test('Supreme Overlord', () => {
          ...
          expect(result.desc()).toBe(
            '31 Atk Kingambit Iron Head vs. 31 HP / 31 Def Aggron: 67-79 (23.8 - 28.1%) -- 91.2% chance to 4HKO'
          );
          kingambit.alliesFainted = 5;
          result = calculate(kingambit, aggron, Move('Iron Head'));
          expect(result.range()).toEqual([100, 118]);
          expect(result.desc()).toBe(
            '31 Atk Supreme Overlord 5 allies fainted Kingambit Iron Head vs. 31 HP / 31 Def Aggron: 100-118 (35.5 - 41.9%) -- guaranteed 3HKO'
          );
